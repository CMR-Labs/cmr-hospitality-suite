...
async def get_guests(hotel_id: UUID, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Guest).where(Guest.hotel_id == hotel_id))
    return result.scalars().all()

@router.post("/", response_model=GuestResponse)
async def create_guest(hotel_id: UUID, data: GuestCreate, db: AsyncSession = Depends(get_db)):
...
async def update_guest(guest_id: UUID, data: GuestUpdate, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Guest).where(Guest.id == guest_id))
    guest = result.scalar_one_or_none()
    if not guest:
        raise HTTPException(status_code=404, detail="Guest not found")
